...
sample1 <- GreenHouse[GreenHouse$GroupID == "sample1", ]
```
...
**Data**: In this tutorial, we will use the [data](https://stat2games.sites.grinnell.edu/data/greenhouse/greenhouse.php) from the [Greenhouse Game](https://stat2games.sites.grinnell.edu/), which provides yielding information in a virtual greenhouse. 
...
# The code below used the favstats() command to see summary statistics of 'Money' variable in the  Greenhouse data.
favstats(GreenHouse$Money)
```
<!-- END_SECTION -->
...
If you worked through the previous lab, you have already used an app to create a scatterplot of the sample1 data (there is no data in the group sample1, so the plot is not showing anything).
...
# ggplot(aes(x = Water, y = Yield), data = sample1) +
#   geom_point()
ggplot(aes(x = Water, y = Yield), data = GreenHouse) +
  geom_point() 
```
...
Instead of apply aesthetic to the base layer, we can also apply locally within layers instead of adding to the base layer. This allows for different aesthetics to be applied to different layers of the same plot.
...
### Other Aesthetics
Now, let's look some other ways to adjust the aesthetics of our plot.
...
ggplot(data = GreenHouse, mapping = aes(x = Water, y = Yield, shape = Crop)) +
  geom_point()
```
...
ggplot(aes(x = Water, y = Profit), data = GreenHouse) +
  geom_point()
```
...
myModel <- lm(Yield ~ Water, data = GreenHouse)
res <- resid(myModel)
